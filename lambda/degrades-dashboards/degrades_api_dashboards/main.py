...
def calculate_number_of_degrades(date):
    ...
    file_names = service.list_files_from_S3(prefix=date, bucket_name=os.getenv("REGISTRATIONS_MI_EVENT_BUCKET"))
    for file_name in file_names:
        file = service.get_file_from_S3(bucket_name=os.getenv("REGISTRATIONS_MI_EVENT_BUCKET"), key=file_name)
        if is_degrade(file):
