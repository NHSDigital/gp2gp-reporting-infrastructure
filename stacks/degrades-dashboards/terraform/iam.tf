...
resource "aws_iam_role_policy_attachment" "degrades_lambda_s3_read" {
  role       = aws_iam_role.degrades_lambda_role.name
  policy_arn = aws_iam_policy.read_registrations_mi_events.arn
}

resource "aws_iam_policy" "read_registrations_mi_events" {
  name   = "${var.environment}-${var.degrades_lambda_name}"
