    ...
    "widgets" : [
      ...
      },
      {
        ...
        "properties" : {
          ...
          "query" : "SOURCE '${data.aws_ssm_parameter.cloud_watch_log_group.value}/spine-exporter*' |  stats count(event) as count by bin(1d) as timestamp | filter event='UPLOADED_CSV_TO_S3'",
          "view" : "timeSeries",
        }
      },
      {
