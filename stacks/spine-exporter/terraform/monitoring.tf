    ...
    "widgets" : [
      ...
      },
      {
        ...
        "properties" : {
          ...
          "query" : "SOURCE '${data.aws_ssm_parameter.cloud_watch_log_group.value}' | stats sum(soze_in_bytes) by bin(1h) | filter strcontains(@logStream, 'spine-exporter') and event='SPINE_EXTRACT_SIZE_BYTES'",
          "view" : "timeSeries"
        }
      },
      {
      ...
      },
      {
        ...
        "properties" : {
          ...
          "query" : "SOURCE '${data.aws_ssm_parameter.cloud_watch_log_group.value}' | stats sum(size_in_bytes) by bin(1d) | filter strcontains(@logStream, 'spine-exporter') and event='SPINE_EXTRACT_ROW_COUNT'",
          "view" : "timeSeries"
        }
      },
      {
      ...
      },
      {
        ...
        "properties" : {
          "period" : 120
          "region" : var.region,
          "title" : "Successful upload count - test source",
          "query" : "SOURCE '${data.aws_ssm_parameter.cloud_watch_log_group.value}' |  stats count(event) as count by bin(1d) as timestamp | filter strcontains(@logStream, 'spine-exporter') event='UPLOADED_CSV_TO_S3'",
          "view" : "timeSeries",
        }
      },
      {
