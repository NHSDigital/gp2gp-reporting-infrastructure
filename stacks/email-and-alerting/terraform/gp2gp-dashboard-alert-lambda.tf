...
resource "aws_lambda_function" "gp2gp_dashboard_alert_lambda" {
  ...
  environment {
    variables = {
      ...
      GP2GP_DASHBOARD_STEP_FUNCTION_URL         = "https://${data.aws_region.current.region}.console.aws.amazon.com/states/home#/statemachines/view/arn:aws:states:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:stateMachine:dashboard-pipeline",
