...
data "aws_iam_policy_document" "ssm_put_access" {
  statement {
    ...
    resources = [
      "arn:aws:ssm:${data.aws_region.current.id}:${local.account_id}:parameter${var.national_metrics_s3_path_param_name}",
      "arn:aws:ssm:${data.aws_region.current.id}:${local.account_id}:parameter${var.practice_metrics_s3_path_param_name}",
    ]
  }
}
